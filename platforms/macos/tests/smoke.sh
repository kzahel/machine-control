...
/usr/bin/swiftc -typecheck guests/macos/ui/macui.swift
...
bin/macvm ui health | /usr/bin/jq -e '.accessibilityTrusted == true' >/dev/null
bin/macvm ui apps >/dev/null
bin/macvm ui windows --app Finder >/dev/null
bin/macvm ui tree --app Finder --interactive --depth 6 --limit 100 >/dev/null

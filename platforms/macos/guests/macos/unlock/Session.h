...
#ifndef STATE_DIR
#define STATE_DIR "/var/db/machine-control-unlock"
#endif
#define GRANT_PATH STATE_DIR "/grant.plist"

static NSDictionary *consoleSessionState(id locked, id users) {
    ...
    if (active.count == 0) return @{@"desktopState": @"no_session"};
    ...
    return @{@"desktopState": [locked boolValue] ? @"locked" : @"unlocked", @"locked": locked, @"uuid": uuid, @"uid": uid,
...
static NSDictionary *sessionState(void) {
    ...
    return consoleSessionState(locked, users);
}
...
static void recordDecision(const char *reason) {
    int fd = open(STATE_DIR "/decisions.log",
        O_WRONLY | O_TRUNC | O_CREAT | O_NOFOLLOW, 0600);
    if (fd >= 0) {
        dprintf(fd, "%s\n", reason);
        close(fd);
    }
...
static BOOL consumeGrant(void) {
    ...
    NSDictionary *broker = [NSDictionary dictionaryWithContentsOfURL:
        [NSURL fileURLWithPath:@STATE_DIR "/broker.plist"] error:NULL];
    NSDictionary *receipt = [NSDictionary dictionaryWithContentsOfURL:
        [NSURL fileURLWithPath:@STATE_DIR "/receipt.plist"] error:NULL];
    BOOL allowed = [grant isKindOfClass:NSDictionary.class] &&
        [receipt[@"enabled"] boolValue] &&
        [grant[@"brokerEpoch"] isEqual:broker[@"epoch"]] &&
        [broker[@"pid"] intValue] > 0 && !kill([broker[@"pid"] intValue], 0) &&
        [grant[@"peerPID"] intValue] > 0 && !kill([grant[@"peerPID"] intValue], 0) &&

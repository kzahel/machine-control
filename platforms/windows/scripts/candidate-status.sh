...
if [[ "${1:-}" != "--json" || $# -ne 1 ]]; then
    printf 'Usage: winvm candidate-status --json\n' >&2
    exit 2
fi

assertion="$($PROVIDER assert-target inspect --json)"
if [[ "$(jq -r '.role' <<<"$assertion")" != candidate ]]; then
    printf 'The exact target is not classified as a candidate\n' >&2
    exit 1
fi

inventory="$($WINVM_REPO_DIR/providers/$WINVM_PROVIDER/workspace.sh \

...
| [`platforms/macos`](platforms/macos/README.md) | Authoritative Tart lifecycle/COW-workspace policy, minimized doctor, selected guest administration, the macOS resident, and its explicitly installed existing-session unlock helper/plug-in | Tart screenshot, pointer, and keyboard remain explicit recovery routes |
...
| [`platforms/ios`](platforms/ios/README.md) | Explicit physical-iPhone selection, signing lifetime, typed common iOS operations, CoreDevice lifecycle/crash reports, Agent Device semantic XCTest, libimobiledevice system logs, screenshots, input, leases, and recovery | The agent runs on the Mac; a stock iPhone cannot host a general YA worker |
...
| [libimobiledevice](research/providers/libimobiledevice.md) | Narrow adopted physical-iOS `os_trace_relay` provider behind bounded transactional system-log artifacts | Replacement for CoreDevice/XCTest, general device shell, or duplicate owner of app/crash/file operations |
